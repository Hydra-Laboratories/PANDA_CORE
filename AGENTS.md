...
- **Deck configuration (YAML)**: Deck layout is defined in a **deck YAML** file (labware only; no gantry settings). Strict schema: only allowed fields; missing, extra, or wrong-type fields raise `ValidationError`.
  - **`src/labware/deck/schema.py`**: Pydantic models for deck YAML: `DeckSchema` (root, single key `labware`), `WellPlateEntry` (two-point calibration points under `calibration.a1` and `calibration.a2`, axis-aligned only), `VialEntry` (single vial location). Both require `model_name`. All use `extra='forbid'`.
