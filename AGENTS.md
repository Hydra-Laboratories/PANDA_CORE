...
- **Labware models** (`src/deck/labware/`):
  ...
  - **`well_plate_holder.py`**: `WellPlateHolder(HolderLabware)` with the `SlideHolder_Top` dimensions and seat-height metadata. Exposes `well_plate: Optional[WellPlate]` (typed reference to the held plate, populated by the loader) and `get_plate_top_z()`.
  - **`vial_holder.py`**: `VialHolder(HolderLabware)` with the `9VialHolder20mL_TightFit` dimensions, seat-height metadata, and slot-count validation. Exposes `vials: Dict[str, Vial]` (typed references to held vials, keyed by vial name) and `get_vial_top_z(name)`.
  ...
  - **`well_plate.py`**: `WellPlate(Labware)` for multi-well plates (e.g., SBS 96-well). Required fields include `name`, `model_name`, dimensions, layout (`rows`, `columns`), `wells`, and volume fields (`capacity_ul`, `working_volume_ul`). Also provides `get_well_center(well_id)`. Carries an optional non-serialized `holder: Optional[WellPlateHolder]` back-reference set by the deck loader.
  - **`vial.py`**: `Vial(Labware)` for a single vial. Required fields include `name`, `model_name`, geometry (`height_mm`, `diameter_mm`), single `location` (bottom-center), and volume fields (`capacity_ul`, `working_volume_ul`). Exposes `get_bottom_center()` and `get_top_center()` helpers, plus an optional non-serialized `holder: Optional[VialHolder]` back-reference set by the deck loader.
- **Deck configuration (YAML)**: Deck layout is defined in a deck YAML file (labware only; no gantry settings). Strict schema: only allowed fields; missing, extra, or wrong-type fields raise `ValidationError`.
  - **`src/deck/yaml_schema.py`**: Pydantic models for deck config files: `DeckYamlSchema` (root, single key `labware`), `WellPlateYamlEntry` (two-point calibration points under `calibration.a1` and `calibration.a2`, axis-aligned only), `VialYamlEntry` (single vial location), `TipRackYamlEntry` (explicit tip pickup coordinates), plus holder entries for `tip_disposal`, `well_plate_holder`, and `vial_holder`. `VialHolderYamlEntry.vials` is `List[str]` (names of top-level vial entries held by the holder); `WellPlateHolderYamlEntry.well_plate` is `Optional[str]` (name of a top-level well_plate entry). All use `extra='forbid'`.
  - **`src/deck/loader.py`**: `load_deck_from_yaml(path, total_z_height=None)` loads a deck YAML config and returns a `Deck` containing all labware. Two-pass: pass 1 builds non-holder labware (and holders' own geometry); pass 2 resolves holder→labware references by name, populates typed `vials`/`well_plate` attributes and the generic `contained_labware` mirror, sets the `.holder` back-reference on each contained labware, and validates that `vial.location.z` (or plate A1 z) matches `holder.location.z + holder.labware_seat_height_from_bottom_mm`. A `DeckLoaderError` is raised on unknown references, wrong-type references, duplicate ownership, or z drift.
...
- **Sample configs**:
  ...
  - `configs/deck/panda_deck.yaml` — YAML deck config derived from `panda.json`, including two 2x15 tip racks, a well plate holder referencing a top-level plate, and a 9-vial holder referencing nine top-level vials.
- **Usage**: Load a deck with `load_deck_from_yaml("configs/deck/deck.sample.yaml", total_z_height=<float>)` or `load_deck_from_yaml("configs/deck/panda_deck.yaml", total_z_height=<float>)` to get a `Deck` object. Access labware: `deck["plate_1"]`. Resolve targets: `deck.resolve("plate_1.A1")` or dotted targets like `deck.resolve("well_plate_holder.panda_plate.A1")` for absolute XYZ.
