...
- **`calibrate_deck_origin.py`**: FLB/BRT deck-origin and bounds calibration utility. Requires explicit `cnc.calibration_homing.runtime_brt` and `origin_flb` profiles in the gantry YAML and refuses to infer `$3` or `$23`. It snapshots rollback GRBL settings, unlocks an initial alarm if present, applies the FLB profile, homes, activates G54, clears `G92`, and sets `G10 L20 P1 X0 Y0 Z0`. It then disables stale soft limits, moves to an estimated BRT inspection pose from configured working-volume maxima minus 2 mm, and programs `$130/$131/$132` from those conservative spans. It restores the runtime BRT profile before disconnecting without running BRT `$H`; BRT is not used to discover or verify machine dimensions. If `--instrument` is supplied, the script then calibrates that instrument's `offset_x`, `offset_y`, `depth`, and `reach_limits` (`gantry_x_min`, `gantry_x_max`, `tcp_z_min`). Runtime protocol `home` remains normal BRT `$H`; FLB homing is calibration/admin only.
    - **Guided usage**: `python setup/calibrate_deck_origin.py --gantry configs/gantry/cub_xl_asmi.yaml --instrument asmi`
    - **Machine-bounds-only first run**: `python setup/calibrate_deck_origin.py --gantry configs/gantry/cub_xl_asmi.yaml`
    - **Ruler gap vs bottom contact**: selected interactively after jogging the TCP lower Z reference.
    - **Safety**: only use when the configured FLB and BRT homing profiles have been verified for the physical controller and FLB switches. The script updates YAML to deck-origin `working_volume` minima at `0.0`, but it does not prove switch placement or machine clearance in offline validation.
