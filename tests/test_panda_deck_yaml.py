...
def test_repository_panda_deck_yaml_loads_with_expected_reference_points():
    ...
    assert deck.resolve("tip_rack_a.A1").x == pytest.approx(144.0)
    assert deck.resolve("tip_rack_a.A1").y == pytest.approx(22.0)
    ...
    assert deck.resolve("tip_rack_a.O2").y == pytest.approx(141.0)
    assert deck.resolve("tip_rack_a.O2").x == pytest.approx(152.5)
    ...
    assert deck.resolve("well_plate_holder.plate.B1").y == pytest.approx(197.0)

    assert isinstance(deck["vial_holder"], VialHolder)
    assert deck.resolve("vial_holder.vial_1").z == pytest.approx(182.0)
    assert deck.resolve("vial_holder.vial_9").y == pytest.approx(284.0)
