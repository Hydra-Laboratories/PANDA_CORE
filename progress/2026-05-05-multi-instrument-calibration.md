...
- Removed the first-time calibration seed workflow; calibration now reads the target gantry YAML directly.
...
- `python setup/calibrate_gantry.py --help` → shows the unified positional gantry input and optional `--output-gantry` entrypoint
- `PYTHONPATH=src python - <<'PY' ... load_gantry_from_yaml(configs/gantry/*.yaml) ... PY` → candidate gantry YAML files load and expose expected instrument names
