...
## YAML-specific schema naming refactor

### Task
Rename deck schema file and classes to YAML-specific names so future readers can immediately distinguish YAML input models from runtime models.

### Work completed
- **File rename**: `src/labware/deck/schema.py` -> `src/labware/deck/yaml_schema.py`
- **Class renames** (clean break, no aliases):
  - `DeckSchema` -> `DeckYamlSchema`
  - `WellPlateEntry` -> `WellPlateYamlEntry`
  - `VialEntry` -> `VialYamlEntry`
  - `LabwareEntry` -> `LabwareYamlEntry`
  - `_Point3D` -> `_YamlPoint3D`
  - `_CalibrationPoints` -> `_YamlCalibrationPoints`
- **Updated imports**:
  - `src/labware/deck/loader.py`: imports from `yaml_schema` with new class names
  - `src/labware/deck/__init__.py`: exports new names only
- **Updated documentation**:
  - `AGENTS.md`: updated file path and class name references

### Validation
- Targeted: `pytest tests/test_deck_loader.py -q` and `pytest tests/test_labware.py -q`
- Full suite: `pytest tests -q`
- Sample script: `python show_deck_objects.py`

### Notes
- Naming-only clean break; no behavioral changes.
- Any external imports using old names will fail and must be updated immediately.

---

## Deck-contains-labware package restructure

### Task
Restructure so the `deck` package is the top-level module containing `labware` as a subpackage, instead of the previous `labware` containing `deck`.

### Work completed
- **Directory restructure**:
  - `src/labware/` removed entirely
  - `src/deck/` is now the top-level package
  - `src/deck/labware/` contains labware models (`labware.py`, `well_plate.py`, `vial.py`)
  - `src/deck/` contains deck-level modules (`loader.py`, `yaml_schema.py`, `errors.py`)
- **Updated imports**:
  - `src/deck/__init__.py`: exports all public symbols (labware models + loader + errors + schema)
  - `src/deck/labware/__init__.py`: exports labware model symbols
  - `src/deck/loader.py`: imports labware from `.labware` subpackage (was `..labware`)
  - `tests/test_labware.py`: `from src.deck import ...` (was `src.labware`)
  - `tests/test_deck_loader.py`: `from src.deck import ...` and `from src.deck.loader import ...` (was `src.labware` / `src.labware.deck.loader`)
  - `show_deck_objects.py`: `from src.deck import ...` (was `src.labware`)
- **Updated documentation**:
  - `AGENTS.md`: updated section header and all file path references
  - `README.md`: updated file path references and import example
...
- All imports via `src.labware` are replaced with `src.deck`.

---

## Deck class: runtime container with target resolution

### Task
Introduce a `Deck` class that wraps the labware dict, providing dict-like access and a `resolve()` method for translating target strings (e.g. `"plate_1.A1"`) into absolute deck coordinates.

### Work completed
- **TDD tests** (`tests/test_deck.py`): 15 tests covering construction, container protocol (`__getitem__`, `__contains__`, `__len__`, `__iter__`), `resolve()` for dotted targets (`plate_1.A1`), bare names (`vial_1`, `plate_1`), and error cases (unknown labware, invalid well).
- **`src/deck/deck.py`**: `Deck` class with:
  - `labware` property exposing the raw dict
  - `resolve(target)` parsing `"labware_key.location_id"` or bare `"labware_key"`
  - Dict-like access: `deck["key"]`, `"key" in deck`, `len(deck)`, `iter(deck)`
- **Loader updated** (`src/deck/loader.py`):
  - `load_labware_from_deck_yaml()` renamed to `load_deck_from_yaml()`, returns `Deck`
  - `load_labware_from_deck_yaml_safe()` renamed to `load_deck_from_yaml_safe()`, returns `Deck`
- **Consumers updated**:
  - `show_deck_objects.py`: uses `load_deck_from_yaml_safe`, iterates via `deck.labware.items()`
  - `tests/test_deck_loader.py`: updated imports and assertions for `Deck` return type
- **Exports updated** (`src/deck/__init__.py`): exports `Deck`, new loader names
- **Documentation updated**: `AGENTS.md` and `README.md` reflect `Deck` class and new API

### Validation
- `pytest tests/test_deck.py -q` -> 15 passed
- `pytest tests/test_deck_loader.py -q` and `pytest tests/test_labware.py -q`
- Full suite: `pytest tests -q`
- Sample script: `python show_deck_objects.py`

### Design decisions
- `Deck` is a plain class (not Pydantic model) — it's a runtime container, not a data schema.
- `resolve()` splits on `.` to get labware key + location ID; bare names call `get_initial_position()`.
- Old loader function names removed entirely (clean break, no aliases).
