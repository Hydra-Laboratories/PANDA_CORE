...
_LabwareUnion = Union[WellPlate, Vial, TipRack]
...
class Deck:
    ...
    def __init__(self, labware: Dict[str, _LabwareUnion]) -> None:
        self._labware = dict(labware)

    @property
    def labware(self) -> Dict[str, _LabwareUnion]:
    ...
    def _get_labware(self, key: str) -> _LabwareUnion:
        try:
            return self._labware[key]
        except KeyError:
            raise KeyError(f"No labware '{key}' on deck.") from None

    def __getitem__(self, key: str) -> _LabwareUnion:
