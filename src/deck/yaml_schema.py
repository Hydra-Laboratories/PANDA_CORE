...
class WellPlateHolderYamlEntry(_BaseHolderYamlEntry):
    ...
    well_plate: Optional[str] = Field(
        default=None,
        description="Name of a top-level well_plate labware entry held by this holder.",
    )

    @model_validator(mode="after")
    def _validate_well_plate_reference(self) -> "WellPlateHolderYamlEntry":
        if self.well_plate is not None and not self.well_plate.strip():
            raise ValueError("well_plate reference must be a non-empty string.")
        return self
...
class VialHolderYamlEntry(_BaseHolderYamlEntry):
    ...
    vials: List[str] = Field(
        default_factory=list,
        description="Names of top-level vial labware entries held by this holder.",
    )
    ...
    def _validate_slot_capacity(self) -> "VialHolderYamlEntry":
        ...
        for vial_name in self.vials:
            if not vial_name.strip():
                raise ValueError("vial reference must be a non-empty string.")
            if vial_name in seen:
                raise ValueError(
                    f"vial '{vial_name}' listed more than once in vial_holder.vials."
                )
            seen.add(vial_name)
        return self
