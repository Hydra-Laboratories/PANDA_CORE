...
def setup_protocol(
    ...
    Steps:
        1. Load machine config (working volume, homing strategy)
        2. Load deck (labware positions)
        3. Load board (instruments with offsets)
        4. Load protocol (command steps)
        5. Validate all deck positions within machine bounds
        6. Validate all gantry positions within machine bounds
        7. Return (Protocol, ProtocolContext)

    Args:
        machine_path: Path to machine YAML config.
    ...
    Raises:
        MachineLoaderError: If machine YAML is invalid or missing.
        ...
        SetupValidationError: If any positions violate machine bounds.
    """
    machine = _load_machine(machine_path)
    ...
    violations = validate_deck_positions(machine, deck)
    violations.extend(validate_gantry_positions(machine, deck, board))
    if violations:
        raise SetupValidationError(violations)

    context = ProtocolContext(board=board, deck=deck, machine=machine)
    return protocol, context


def _load_machine(path: str | Path):
    """Load machine config with user-friendly errors."""
    try:
        return load_machine_from_yaml(path)
    except Exception as exc:
        if isinstance(exc, MachineLoaderError):
            raise
        raise MachineLoaderError(str(exc)) from exc
